  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={sanitizeHtml(post.excerpt?.rendered || 'Читайте нашу статью', {
            allowedTags: [],
            allowedAttributes: {},
          })}
        />
            ...
            <div className={styles.post__navigation}>
              ...
              <div className={styles.post__social}>
                ...
                <div className={styles.social__links}>
                  {/* Примечание: window.location.href недоступен на сервере.
                      Для статического сайта эти ссылки будут корректно работать в браузере,
                      но при SSR/SSG значение href может быть некорректным.
                      Можно использовать клиентский рендеринг для этих ссылок. */}
                  <a
                    href={`https://t.me/share/url?url=&text=${encodeURIComponent(post.title.rendered)}`} // URL будет заполнен браузером
                    target="_blank"
                  ...
                  <a
                    href={`https://api.whatsapp.com/send?text=${encodeURIComponent(post.title.rendered + ' ')}`} // URL будет добавлен браузером
                    target="_blank"
                    rel="noopener noreferrer"
                    className={styles.social__link}
                  ...
                  <a
                    href={`https://vk.com/share.php?url=&title=${encodeURIComponent(post.title.rendered)}&noparse=true`} // URL будет заполнен браузером
                    target="_blank"
                    rel="noopener noreferrer"
                    className={styles.social__link}
                  ...
                  <a
                    href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title.rendered)}&url=`} // URL будет заполнен браузером
                    target="_blank"
                    rel="noopener noreferrer"
                    className={styles.social__link}
...
// Явно указываем тип для возвращаемого значения
export const getStaticProps: GetStaticProps<PostPageProps> = async ({ params }: GetStaticPropsContext) => {
  try {
    const slug = params?.slug as string;
    // Проверка на существование slug для дополнительной безопасности
    if (!slug) {
    ...
    // return { props: { post: null, error: 'Не удалось загрузить пост' } };
  }
};